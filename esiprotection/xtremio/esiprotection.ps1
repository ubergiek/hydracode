...
$script:SnapShotType = 'vnxsnapshot'    #'vnxsnapshot|snapview', ignored if the storageSystemType is set to 'xtremio'
...
function Import-EsiConfig
{
    ...
    else
    {
        Try
        {
            ...
            $script:xtremioRpCgName = $EsiHostConfig.esiprotection.xtremio_consistencygroup
...
if ($storageSystem -eq $null -or $RpSystem -eq $null)
{
        ...
        foreach ($RpConsistencyGroup in $RpConsistencyGroups)
        {
            ...
            else
            {
                ...
                {    
                    ...
                    foreach ($EsiHost in $CgEsiHosts)
                    {
                        if ($script:storageSystemType -eq 'vnx')
                        {
                            Remove-PreviousSnapshots -EsiHost $EsiHost
                        }
                        else
                        {
                            Write-logfile -message "XtremIO Storage detected. Skipping Snapshot removal."
                        }
